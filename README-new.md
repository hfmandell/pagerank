...
Creating a simple search engine for the website <https://www.lawfareblog.com> improved by word2vec. The website provides legal analysis on US national security issues.
...
The `WebGraph.power_method` function in `pagerank.py` computes the pagerank vector.
...
Example outputs of using `--search_query`:
...
Running the following four commands:
```
...
```
Notice that the last command takes considerably more iterations to compute the pagerank vector.
...
The `WebGraph.make_personalization_vector` function outputs a personalization vector tuned for the input query.
